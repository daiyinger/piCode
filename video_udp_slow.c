...
void *pthread_snd(void *socketsd)
{
		...
		char *senduf=(char *)calloc(1,1024*5);
		...
		while(1)
		{
				...
				for(len = 0 ; len < img_len ; len += 1024)
				{
					if((img_len-len) > 1024)
					{
						...
						ck = senduf[0]+senduf[10]+senduf[1023];
						senduf[1025]=ck;
						ret = sendto(g_udp_fd,senduf,1024+2,0,(struct sockaddr *)&g_addr,sizeof(struct sockaddr_in)); 
					}
					else
					{
